...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => (
            visible.value ?
                <CollapsePanel name="ElementUserTask" v-slots={{
                    header: () => '任务配置',
                    default: () => (
                        <>
                            {formProp.value &&
                                <Form ref={formRef} model={formProp.value} autocomplete="off" labelCol={{span: 6}}
                                      validateTrigger='blur'>
                                    <FormItem label="任务人" name="assignee">
                                        <InputGroup compact>
                                            <Input style="width: calc(100% - 40px)"
                                                   v-model:value={formProp.value.assignee} onChange={updateProps}/>
                                    ...
                                    <FormItem label="到期日" name="dueDate">
                                        <InputGroup compact>
                                            <Input style="width: calc(100% - 40px)"
                                                   v-model:value={formProp.value.dueDate} onChange={updateProps}/>
                                            <Button v-slots={{icon: () => (<Settings size={18}/>)}}/>
                                        </InputGroup>
                                    </FormItem>
                                </Form>
