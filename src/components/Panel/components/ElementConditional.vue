<template>
  <collapse-panel name="element-conditional">
    <template #header>
    ...
    <div class="element-conditional">
      <template v-if="varVisible">
        <edit-item key="variableName" :label="$t('panel.variableName')" :label-width="120">
          <a-input v-model:value="variableName" maxlength:="32" @change="setElementVariableName" />
        </edit-item>
        <edit-item
          ...
          :label-width="120"
        >
          <a-input v-model:value="variableEvents" @change="setElementVariableEvents" />
        </edit-item>
      </template>
      ...
      <edit-item
        ...
        :label-width="120"
      >
        <a-input v-model:value="conditionData.expression" @change="setConditionExpression" />
      </edit-item>
      <template v-if="conditionData.conditionType && conditionData.conditionType === 'script'">
        ...
        <edit-item key="scriptLanguage" :label="$t('panel.scriptLanguage')" :label-width="120">
          <a-input v-model:value="conditionData.language" @change="setConditionScriptLanguage" />
        </edit-item>
        <edit-item
        ...
        <edit-item
          ...
          :label-width="120"
        >
          <a-input v-model:value="conditionData.resource" @change="setConditionScriptResource" />
        </edit-item>
      </template>
    </div>
  </collapse-panel>
...
<script lang="ts">
  ...
  import * as CU from '../../utils/conditionUtil'
  ...
  import {ConditionalForm} from '@/components/types'
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const setElementVariableName = (event: Event) => {
        const value = (event.target as HTMLInputElement).value
        CU.setVariableNameValue(getActive.value!, value)
      }
      const setElementVariableEvents = (event: Event) => {
        const value = (event.target as HTMLInputElement).value
