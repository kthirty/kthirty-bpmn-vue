...
import {CollapsePanel, Form, FormItem, Input, RadioGroup, Select, Textarea} from "ant-design-vue";
...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => (
            visible.value ?
                <CollapsePanel name="ElementCondition" v-slots={{
                    header: () => '流转条件',
                    default: () => (
                        <Form ref={formRef} model={conditionForm.value} autocomplete="off" labelCol={{span: 6}}
                              validateTrigger='blur'>
                            <FormItem required={true} label="类型" name="type">
                                <Select onChange={updateProperties} options={options}
                                        v-model:value={conditionForm.value.type}/>
                            </FormItem>
                            {conditionForm.value.type === 'expression' &&
                                <FormItem required={true} label="条件表达式" name="expression">
                                    <Textarea onChange={updateProperties}
                                              v-model:value={conditionForm.value.expression}/>
                                </FormItem>
                            }
                        </Form>
                    )
