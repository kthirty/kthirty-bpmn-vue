import {defineComponent, ref, onMounted, watch, PropType, markRaw} from "vue";
...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => (
            <CollapsePanel name="basic" v-slots={{
                header: () => '基础属性',
                default: () => (
                    <>
                        {basicInfo.value &&
                            <Form ref={formRef} model={basicInfo.value} autocomplete="off" labelCol={{span: 6}} validateTrigger='blur'>
                                <FormItem required={true} label="ID" name="id"><Input maxlengt={32} v-model:value={basicInfo.value.id} onChange={updateProperties}/></FormItem>
                                <FormItem required={true} label="节点名称" name="name"><Input maxlengt={32} v-model:value={basicInfo.value.name} onChange={updateProperties}/></FormItem>
                                <FormItem label="节点描述" name="doc"><Textarea v-model:value={basicInfo.value.doc} onChange={updateProperties}/></FormItem>
                                {currentIsProcess.value && <FormItem label="版本号" name="version"><Input v-model:value={basicInfo.value.version} onChange={updateProperties}/></FormItem>}
                                {currentIsProcess.value && <FormItem label="可执行" name="executable"><Switch v-model:checked={basicInfo.value.executable} onChange={updateProperties}/></FormItem>}
                            </Form>}
                    </>
                ),
            }}/>
